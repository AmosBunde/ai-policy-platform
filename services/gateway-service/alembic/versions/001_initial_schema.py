...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "crawl_frequency_minutes > 0", name="ck_crawl_frequency_positive"
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "source_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("regulatory_sources.id"),
            nullable=True,
        ),
        ...
        sa.Column(
            "raw_metadata", postgresql.JSONB(), server_default=sa.text("'{}'::jsonb")
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "key_changes", postgresql.JSONB(), server_default=sa.text("'[]'::jsonb")
        ),
        sa.Column(
            "classification", postgresql.JSONB(), server_default=sa.text("'{}'::jsonb")
        ),
        sa.Column(
            "impact_scores", postgresql.JSONB(), server_default=sa.text("'{}'::jsonb")
        ),
        ...
        sa.Column(
            "effective_dates", postgresql.JSONB(), server_default=sa.text("'[]'::jsonb")
        ),
        sa.Column("urgency_level", sa.String(20), server_default="normal"),
        sa.Column("confidence_score", sa.Float(), server_default="0.0"),
        sa.Column(
            "token_usage", postgresql.JSONB(), server_default=sa.text("'{}'::jsonb")
        ),
    ...
    op.create_index(
        "idx_enrichments_urgency", "document_enrichments", ["urgency_level"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "document_ids", postgresql.JSONB(), server_default=sa.text("'[]'::jsonb")
        ),
